...
/**
 * Tests for event attribute grammar patterns in both blits.json and embedded-html.json.
 *
 * Event attributes (@loaded, @error, @updated) accept two valid forms:
 *   1. $handlerName reference:  @loaded="$myHandler"
 *   2. Arrow function:          @loaded="(dims) => $handleLoad(dims)"
 *
 * Anything else should be marked as invalid.illegal.handler-value.
 *
 * The grammar uses 4 ordered patterns:
 *   Pattern 1: match $handler reference (exact match)
 *   Pattern 2: begin/end with lookahead for => (arrow function)
 *   Pattern 3: catch-all (invalid handler value)
 *   Pattern 4: invalid event name (e.g. @click, @focus)
 */

// Loads and parses a grammar JSON file from the syntaxes directory
...
// For blits.json: patterns live in repository.blits-event-attributes.patterns.
// For embedded-html.json: patterns are nested inside the tag begin/end block.
function getEventPatterns(grammar) {
  ...
  if (templatePattern) {
    const tagPattern = templatePattern.patterns.find(
      (p) => p.begin && p.begin.startsWith('(<)')
    )
    if (tagPattern) {
      // Return all event attribute patterns (both valid and invalid event names)
      // Identified by the @(?:loaded|error|updated) or @(?!loaded patterns
...
// Simulates TextMate ordered pattern matching — returns index of first matching pattern, or -1.
...
function runGrammarSuite(grammarName, filename) {
  suite(`Event Attribute Grammar Tests (${grammarName})`, () => {
    ...
    test('Should have 4 event attribute patterns in correct order', () => {
      // Pattern 0: $handler match (valid event name + $ref value)
      // Pattern 1: arrow function begin/end with => lookahead (valid event name + arrow fn)
      // Pattern 2: catch-all invalid handler value (valid event name + bad value)
      // Pattern 3: invalid event name (e.g. @click, @focus)
      assert.strictEqual(patterns.length, 4, `Expected 4 event patterns, got ${patterns.length}`)

      // Pattern 0 should be a match pattern for $handler
      assert.ok(patterns[0].match, 'Pattern 0 should be a match pattern')
      assert.ok(
        patterns[0].match.includes('[$]'),
        'Pattern 0 should match $ handler references'
      )

      // Pattern 1 should be a begin/end pattern with => lookahead
      assert.ok(patterns[1].begin, 'Pattern 1 should be a begin/end pattern')
      assert.ok(
        patterns[1].begin.includes('=>'),
        'Pattern 1 should have => lookahead in begin regex'
      )

      // Pattern 2 should be the catch-all invalid handler value
      assert.ok(patterns[2].match, 'Pattern 2 should be a match pattern')
      assert.ok(
        patterns[2].captures &&
          (patterns[2].captures['2'].name.includes('invalid.illegal') ||
            patterns[2].name.includes('invalid')),
        'Pattern 2 should scope as invalid.illegal'
      )

      // Pattern 3 should catch invalid event names (e.g. @click)
